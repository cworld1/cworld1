...
::: collapse 方案2 云函数运行（推荐）

> 注意：`v1.5.0+` 版本增加了随机延迟功能，默认为10-300s。故云函数的`执行超时时间`应设置为大于变量中的 `MAX_SLEEP_SECS`（最大休眠秒数）。
...
> 如果你是国际服则应该配置 `COOKIE_HOYOLAB`而不是 `COOKIE_MIHOYOBBS`；不使用Discord推送，可以不配置 `DISCORD_WEBHOOK`。
...
::: collapse 方案4 使用 PyPI Package 运行

使用 [pypi包](https://pypi.org/project/genshinhelper/) 进行安装：
...
<a href="javascript:(function(){let domain=document.domain;let cookie=document.cookie;prompt('Cookies: '+domain, cookie)})();">获取Cookie</a>
...
在抓包结果请求头（地址 query 参数）里提取出aid、s、gsid 和 from 参数，组合成形如 `"aid=xxx; s=xxx;gsid=xxx;from=xxx"`的形式填入。

`COOKIE_KA` 用来追踪超话活动和自动提取兑换码。事实上会在抓取 `COOKIE_WEIBO`  的时候会一起被抓取到。注意 Cookie 需要`SUB`和`SUBP`两个字段。填写形式同 `COOKIE_WEIBO`。

详细抓取教程可参考视频：[微博国际版iOS抓包教程](https://b23.tv/3cj7vG)
